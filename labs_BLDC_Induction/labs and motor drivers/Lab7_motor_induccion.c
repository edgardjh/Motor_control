...
//global variables
float velocidad_VR1=1953; // inicializa en este valor por que es igual a 1 Hz
...
void main(void)
{
	...
	ConfigCpuTimer(&CpuTimer0,150,velocidad_VR1);
	IER |= 3;
	EINT;
	ERTM;
...

{
	...
	static float v = 0;
	 // factor de 0 a 1    =0.016666

		v = (amp*(velocidad_VR1+1))*(EPwm1Regs.TBPRD -_IQsat( _IQ30mpy((sine_table[index_a]+_IQ30(0.9999))/2, EPwm1Regs.TBPRD), EPwm1Regs.TBPRD,0));

		EPwm1Regs.CMPA.half.CMPA = (int)v;

		EPwm2Regs.CMPA.half.CMPA = EPwm2Regs.TBPRD -_IQsat( _IQ30mpy((sine_table[index_b]+_IQ30(0.9999))/2, EPwm2Regs.TBPRD), EPwm2Regs.TBPRD,0);

		EPwm3Regs.CMPA.half.CMPA = EPwm3Regs.TBPRD -_IQsat( _IQ30mpy((sine_table[index_c]+_IQ30(0.9999))/2, EPwm3Regs.TBPRD), EPwm3Regs.TBPRD,0);

		if(index_a++ > 511) index_a = 0;
		if(index_b++ > 511) index_b = 0;
		if(index_c++ > 511) index_c = 0;

	tiempo = 1953-(32.55*velocidad_VR1);
	ConfigCpuTimer(&CpuTimer0,150,tiempo); //1953 = 1Hz; 32.55 = 60Hz. 1953/60=32.55. vel=0:59 >> 32.55<<1953
...
interrupt void adc_isr()
{
	...
	//velocidad_VR1 = (0.47*velocidad_VR1)+32;
	velocidad_VR1 = _IQsat(AdcMirror.ADCRESULT0,59,1); //funcion que sust. los 2 comandos anteriores
